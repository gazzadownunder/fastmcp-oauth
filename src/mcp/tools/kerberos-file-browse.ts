...
const ListDirectorySchema = z.object({
  path: z
    .string()
    .describe(
      'SMB path to list (e.g., //192.168.1.25/share/folder or \\\\fileserver\\share\\folder)'
    ),
...
export const createListDirectoryTool: ToolFactory = (context: CoreContext) => ({
  ...
  handler: async (params: ListDirectoryParams, mcpContext: MCPContext): Promise<LLMResponse> => {
    ...
    try {
      ...
      if (!kerberosModule) {
        throw new Error('Kerberos delegation module not available. Check server configuration.');
      }
      ...
      const ticketResult = await kerberosModule.delegate(mcpContext.session!, 's4u2proxy', {
        targetSPN: spn,
      });
